...
# We start rosbag record a few seconds before
# we simulate the scenario (rosbag startup can miss a few messages)
ROSBAG_STARTUP_DELAY = 2.0
ROSBAG_DURATION_AFTER_RUN = 3.0
...
class ScenarioBase(object):
    """
    Base for profiler scenarios. setup() runs before the rosbag record is started;
    run() runs after start_rosbag_record() to simulate scenario of interest
    """
...
def run_scenario_with_rosbag(
    ...
    """
    Common runner: setup -> start_rosbag -> -> run -> stop_rosbag.
    ...
    try:
        ...
        rospy.sleep(ROSBAG_DURATION_AFTER_RUN)
        return ret
    finally:
        stop_rosbag_record(rosbag_proc)


# Scenario: Running -> Error (Sensor-Triggered Error)
class RunToErrorScenario(ScenarioBase):
    """
    Running-to-error scenario.
    setup() gets system to IDLE then RUNNING.
    run() injects error.
    """
...
class ErrorToRunScenario(ScenarioBase):
    """
    Error-to-running (recovery) scenario.
    setup() gets system to IDLE, RUNNING, then ERROR.
    run() clears error, waits full recovery.
    """
...
def _good_predicate_temperature(msg):
    return getattr(msg, "temperature", 100.0) <= 55.0
...
def _error_predicate_temperature(msg):
    return getattr(msg, "temperature", 0.0) > 55.0

def _error_predicate_gps(msg):
    return getattr(msg, "accuracy", 0.0) > 200.0

def _error_predicate_signal_low(msg):
    return getattr(msg, "strength", 1) == SignalStrengthStamped.LOW_SIGNAL

def _error_predicate_signal_not_connected(msg):
    return getattr(msg, "strength", 1) == SignalStrengthStamped.NOT_CONNECTED
