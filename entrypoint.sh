...
SMART_POLARIS_WS="/home/ros/smart_polaris_ws"
...
if [ $(stat -c "%u" "${POLARIS_WS}") -eq 0 ] || [ $(stat -c "%u" "${SMART_POLARIS_WS}") -eq 0 ]; then
    sudo chown -R ros:ros "${POLARIS_WS}" && sudo chown -R ros:ros "${SMART_POLARIS_WS}"
fi

source "${SMART_POLARIS_WS}/devel/setup.bash"
